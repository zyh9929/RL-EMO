...
def get_train_valid_sampler(trainset, valid=0.1, dataset='IEMOCAP',select_rate=1):
    size = int(len(trainset)*select_rate)
    idx = list(range(size))
    split = int(valid*size)
    return SubsetRandomSampler(idx[split:size]), SubsetRandomSampler(idx[:split])


def get_MELD_loaders(batch_size=32, valid=0.1, num_workers=0, pin_memory=False,select_rate=1):
    #trainset = MELDDataset('ERC_features/MELD_features2.pkl')
    trainset = MELDDataset()
    train_sampler, valid_sampler = get_train_valid_sampler(trainset, valid, 'MELD',select_rate=select_rate)
    ...
    #testset = MELDDataset('ERC_features/MELD_features2.pkl', train=False)
...
def get_IEMOCAP_loaders(batch_size=32, valid=0.1, num_workers=0, pin_memory=False,select_rate=1):
    trainset = IEMOCAPDataset()
    train_sampler, valid_sampler = get_train_valid_sampler(trainset, valid,select_rate=select_rate)
...
def get_EmoryNLP_loaders(batch_size=32, valid=0.1, num_workers=0, pin_memory=False,select_rate=1):
    trainset = EmoryNLPDataset("train")
    validset=EmoryNLPDataset("val")
...
def get_DailyDialog_loaders(batch_size=32, valid=0.1, num_workers=0, pin_memory=False,select_rate=1):
    trainset = DailyDialogDataset("train")
    validset=DailyDialogDataset("val")
...
def train_or_eval_model(model, loss_function, dataloader, epoch, optimizer=None, train=False):
    ...
    if args.Dataset in ['IEMOCAP', 'MELD', 'EmoryNLP']:
...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, modals, optimizer=None, train=False,args=None):
    losses, preds, labels = [], [], []
    scores, vids = [], []
    ...
    for data in dataloader:
        ...
        preds.append(torch.argmax(log_prob.cpu(), 1).numpy())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--graph_type', default='DeepGCN', help='relation/GCN3/DeepGCN/MMGCN/MMGCN2')
    ...
    parser.add_argument('--mm_fusion_mthd', default='none', help='method to use multimodal information: concat, gated, concat_subsequently')
    ...
    n_classes = 7 if args.Dataset == 'MELD' else 6 if args.Dataset == 'IEMOCAP' else 7 if  args.Dataset == 'EmoryNLP' else 7 if args.Dataset == 'DailyDialog' else 1
    #n_classes  =5
    ...
    if args.Dataset == 'IEMOCAP':
        loss_weights = torch.FloatTensor([1 / 0.227883,
                                          1 / 0.160585,
                                          1 / 0.086747,
                                          1 / 0.144406,
                                          1 / 0.127711])
    ...
    if args.second_train:
        state = torch.load('save/{}_GRU.pth'.format(args.pretrained_model))
        model.load_state_dict(state)


    test_label = False
    if test_label:
        state = torch.load('save/IEMOCAP+0.3MELD.pth')
        model.load_state_dict(state)
        test_loss, test_acc, test_label, test_pred, test_fscore, _, _, _, _, _ = train_or_eval_graph_model(model, loss_function, test_loader, 0, cuda, args.modals, args=args)
        ...
        for e in range(n_epochs):
            ...
            if args.graph_model:
                train_loss, train_acc, _, _, train_fscore, _, _, _, _, _ = train_or_eval_graph_model(model,
                                                                                                     ...
                                                                                                     optimizer, True,
                                                                                                     args=args)
                valid_loss, valid_acc, _, _, valid_fscore, _, _, _, _, _ = train_or_eval_graph_model(model,
                                                                                                     loss_function,
                                                                                                     valid_loader, e,
                                                                                                     cuda, args.modals,
                                                                                                     args=args)
                test_loss, test_acc, test_label, test_pred, test_fscore, _, _, _, _, _ = train_or_eval_graph_model(
                    model, loss_function, test_loader, e, cuda, args.modals, args=args)
                ...
                valid_loss, valid_acc, _, _, _, valid_fscore, _ = train_or_eval_model(model, loss_function,
                                                                                      valid_loader, e)
            ...
            if best_fscore == None or best_fscore < test_fscore:
                ...
                if args.second_train:
                    torch.save(model.state_dict(), "save/{}+{}{}.pth".format(args.pretrained_model,args.second_rate,args.Dataset))
                else:
                    torch.save(model.state_dict(), "save/" + args.Dataset + '_' + args.base_model + '.pth')
