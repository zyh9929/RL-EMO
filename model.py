...
class Model(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future, 
        ...
        if self.base_model == 'LSTM':
            ...
            self.gru = nn.GRU(input_size=D_m, hidden_size=D_g//2, num_layers=2, bidirectional=True, dropout=dropout)
            if 'a' in self.modals:
                hidden_a = D_g
                self.linear_a = nn.Linear(D_m_a, hidden_a)
                if self.av_using_lstm:
        ...
        if self.graph_type == 'GCN3':
            self.return_feature = True
            if not self.multi_modal:
                self.graph_net = GCN_2Layers(D_g, graph_hidden_size, n_classes, self.dropout, self.use_topic, self.use_residue)
            else:
                if 'a' in self.modals:
                    self.graph_net_a = GCN_2Layers(D_g, graph_hidden_size, D_g, self.dropout, self.use_topic, self.use_residue, self.return_feature)
                if 'v' in self.modals:
                    self.graph_net_v = GCN_2Layers(D_g, graph_hidden_size, D_g, self.dropout, self.use_topic, self.use_residue, self.return_feature)
                if 'l' in self.modals:
                    self.graph_net_l = GCN_2Layers(D_g, graph_hidden_size, D_g, self.dropout, self.use_topic, self.use_residue, self.return_feature)
            ...
            if not self.multi_modal:
                ...
                self.graph_net = GCNII_2(nfeat=D_g, nlayers=Deep_GCN_nlayers, nhidden=100, nclass=n_classes, dropout=self.dropout,
                ...
                if 'a' in self.modals:
                    self.graph_net_a = GCNII_2(nfeat=D_g, nlayers=Deep_GCN_nlayers, nhidden=graph_hidden_size, nclass=n_classes, dropout=self.dropout,
                                             lamda=0.5, alpha=0.1, variant=True, return_feature=self.return_feature, use_residue=self.use_residue,
                                             reason_flag=self.reason_flag)
                if 'v' in self.modals:
                    self.graph_net_v = GCNII_2(nfeat=D_g, nlayers=Deep_GCN_nlayers, nhidden=graph_hidden_size, nclass=n_classes, dropout=self.dropout,
                                             lamda=0.5, alpha=0.1, variant=True, return_feature=self.return_feature, use_residue=self.use_residue,
                                             reason_flag=self.reason_flag)
                if 'l' in self.modals:
                    self.graph_net_l = GCNII_2(nfeat=D_g, nlayers=Deep_GCN_nlayers, nhidden=graph_hidden_size, nclass=n_classes, dropout=self.dropout,
                                             lamda=0.5, alpha=0.1, variant=True, return_feature=self.return_feature, use_residue=self.use_residue,
                                             reason_flag=self.reason_flag)
